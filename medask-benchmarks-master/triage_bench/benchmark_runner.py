...
from medask.ummon.ollama import UmmonOllama
...
def create_client(model: str) -> BaseUmmon:
    ...
    - Ollama models: ollama+<model> or auto-detect common model names (llama*, qwen*, deepseek-r1*, mistral*)
    - KoboldCPP: koboldcpp+http://...
    - Local LLM: http://... or https://...
    """
    # OpenAI models
    ...
    # Explicit Ollama prefix
        ...
        return UmmonOllama(ollama_model)
    
    # KoboldCPP
    elif "koboldcpp+http://" in model or "koboldcpp+https://" in model:
        url = model[len("koboldcpp+"):]
        return UmmonKoboldCPP(url)
    
    # Local LLM server (URL)
    elif model.startswith("http://") or model.startswith("https://"):
        return UmmonLocalLLM(model)
    
    # Auto-detect Ollama models (common open-source model patterns)
    elif _is_ollama_model(model):
        return UmmonOllama(model)
...
def _is_ollama_model(model: str) -> bool:
    """
    Check if model name matches common open-source model patterns.
    
    Auto-detects: llama*, qwen*, deepseek-r1*, mistral* (but not deepseek-chat/reasoner)
    """
    model_lower = model.lower()
    
    # Exclude known cloud models first
    cloud_models = ["deepseek-chat", "deepseek-reasoner"]
    if model_lower in [m.lower() for m in cloud_models]:
        return False
    
    # Common open-source model prefixes
    ollama_patterns = [
        "llama",
        "qwen",
        "deepseek-r1",  # Open-source DeepSeek R1 (not the API deepseek-chat)
        "mistral",
        "mixtral",
    ]
